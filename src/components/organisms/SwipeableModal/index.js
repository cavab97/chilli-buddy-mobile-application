...
import { Text, TouchableOpacity, View, ScrollView } from "../../atoms";

import Modal from "react-native-modal";
import styles from "./styles";

function renderData(type, dataSource, selectedItem, onPress) {
  const { buttonContainer, button, selectedButton, selectedText, text } = styles;
  ...
  switch (type) {
          ...
          {dataSource.map((data) => (
            <TouchableOpacity key={data.id} onPress={() => onPress(data.id)}>
              <View style={selectedItem === data.id ? selectedButton : button}>
                <Text style={selectedItem === data.id ? selectedText : text}>{data.title}</Text>
              </View>
            </TouchableOpacity>
          ))}
        </View>
    ...
    case "tag":
      if (dataSource.length === 0) {
        return (
          <View style={buttonContainer}>
            <Text style={text}>Currently no category is selected.</Text>
          </View>
        );
      } else {
        return (
